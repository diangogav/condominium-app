...
class PettyCashViewModel @Inject constructor(
    ...
    init {
        viewModelScope.launch {
            authRepository.currentUser.collect { user ->
                if (user != null) {
                    // Fix: If buildingId (from currentUnit) is empty, try to get it from the units list
                    // This is common for admins who might not have a specific 'current unit' but belong to a building
        ...
        viewModelScope.launch {
            ...
            _uiState.update { state ->
                state.copy(
                    ...
                    error = if (balanceResult.isFailure) balanceResult.exceptionOrNull()?.message else null
                )
            }
        }
    }
    ...
    fun registerExpense(
        ...
        if (amount > currentBalance) {
            _uiState.update { it.copy(error = "El monto excede el saldo disponible ($currentBalance)") }
            return
        }
        ...
        viewModelScope.launch {
            ...
            val result = pettyCashRepository.registerExpense(
                bId, amount, description, category, evidencePath
            )
            if (result.isSuccess) {
                refreshData()
                onSuccess()
