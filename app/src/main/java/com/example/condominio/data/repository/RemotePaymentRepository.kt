...
@Inject
constructor(private val apiService: ApiService, private val gson: com.google.gson.Gson) :
        PaymentRepository {
            ...
            val response =
                    if (!proofUrl.isNullOrEmpty()) {
                        ...
                        apiService.createPaymentMultipart(
                                ...
                                buildingId =
                                        buildingId?.toRequestBody("text/plain".toMediaTypeOrNull()),
                                method =
                                        method.name.toRequestBody("text/plain".toMediaTypeOrNull()),
                                date = dateStr.toRequestBody("text/plain".toMediaTypeOrNull()),
                                notes = description.toRequestBody("text/plain".toMediaTypeOrNull()),
                                reference =
                                        reference?.toRequestBody("text/plain".toMediaTypeOrNull()),
                                bank = bank?.toRequestBody("text/plain".toMediaTypeOrNull()),
                                allocations =
                                        gson.toJson(allocationDtos)
                                                .toRequestBody(
